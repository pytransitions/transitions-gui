...
class TestCodeFormat(unittest.TestCase):
    def test_conformance(self):
        ...
        if exists('transitions_gui'):  # when run from root directory (e.g. tox)
            style.input_dir('transitions_gui')
    ...
    def test_mypy_package(self):
        call = ['mypy', '--config-file', 'mypy.ini', '--strict', 'transitions_gui', 'tests/test_imports.py']

        # when run from root directory (e.g. tox) else when run from test directory (e.g. pycharm)
        project_root = '.' if exists('transitions_gui') else '..'
